...
That writes `modApiMatrices1` and `modApiMatrices2` plus the rest of Menu13 (`modApiMatrixCreate`, `modApiCholesky`, `modApiEigenDecomp`, `modApiUnitary`, `modApiMatrixUtilities`). Excel VBA Lib → Matrices holds Create / Operations / Properties / Validation / Decompositions (Personal-only tools mixed in; duplicate Personal identity/zeros/ones/exchange/transpose/dot entries are omitted). **Analysis** (Personal Menu18) is SVD, AX=B, confusion, covariance/correlation, and residuals. **Plots Charts** (Personal Menu11) is histograms, regression, capability, distribution plots, QQ, Lorenz/Gini, ACF, Diebold-Mariano, and XmR.
