...
- Matrices (`source/Api/modApiMatrices.bas` + `source/Internal/modInternalMatrices.bas`) — Personal Menu13 create / operate / Cholesky / symmetric eigen / QR / LU, plus `Mat*` worksheet UDFs and `MatrixMultDefined`. Menu **Excel VBA Lib → Matrices**. Personal `.bas` dumps were gitignored; this is an array-based reimplementation of that family (not a line-for-line paste). `scripts/Export-PersonalModules.ps1` exports the original modules when Personal.xlsb is present.
- Matrix pack expansion: scale, integer power, rank, diagonal extract, outer/dot product, 1-/inf-norms, exchange / Toeplitz / Vandermonde create, LU, adjugate, pseudoinverse, orthogonality test. `RegisterMatrixUdfs` lists the `Mat*` names under Insert Function (category **Excel VBA Lib**). `Import-AddinModules.ps1 -All` fails if `modInternalMatrices` / `modApiMatrices` / `modAddinMenu` are missing from the `.xlam` and prints the public matrix names after import.
